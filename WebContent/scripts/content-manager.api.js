...
var ContentManagerAPI = function(){
 ...
 // execute update user action.
    var updateUser = function() {
		...
		var oldPassword = $("div#oldPassword").text();
		var newPassword = $("#newPassword").val();
		var confirmPassword = $("#confirmPassword").val();
		var roleIds = "";
		...
		if( ( ($("#changePassword").is(":checked") && newPassword==confirmPassword && newPassword!="" )
			|| !($("#changePassword").is(":checked")) )
			&& username!=""){
			$.ajax({
				type: "POST",
				url: "update.do",
			...
			$('.alert-danger span').text("用户名不能为空，请重新输入！");
			$('.alert-danger').show();
			success = false;
		}else{
			$('.alert-danger span').text("密码不匹配，请重新输入！");
			$('.alert-danger').show();
			success = false;
		}	
 ...
 // create user.
    ...
    var initElementEvents = function() {
		...
		$('#updateUser-confirm1').click(function() {
	    	updateUser();
	    });
		$('#updateUser-confirm').click(function() {
	    	updateUser();
	    });
	    ...
	    $('#newUser-confirm').click(function(e) {
	    	if(createUser()=="InputError"){
	    		e.preventDefault();
	    	};
	    });
	    ...
	    $("tr.dashboard a.btn").each(function(){
			$(this).click(function(){
				var dashboardId = $(this).attr('id');
				var uic_machine_num = $('#uic_machine_num'+dashboardId).val();
				var uic_prod_id = $("select#"+dashboardId+" option:selected").attr('id');
				var uic_output_plan = $('#uic_output_plan'+dashboardId).val();
				var uic_output_actual = $('#uic_output_actual'+dashboardId).val();
				$.ajax({
		            type: "POST",
		            url: "update1.do",
		            data: {
		            	dashboardId:dashboardId,
		            	uic_machine_num:uic_machine_num,
		            	uic_prod_id:uic_prod_id,
		            	uic_output_plan:uic_output_plan,
		            	uic_output_actual:uic_output_actual
		            	},
		            dataType: "text",
		            success: function (result) {
		            	var responseJson = eval ("(" + result + ")");
		            	if( responseJson.value == "done" ){
		            		$('.alert-success span').text("看板数据已成功修改！");
		       	   	 	 	$('.alert-success').show();
		            	}else{
		            		$('.alert-danger span').text("看板数据修改失败！");
		            		$('.alert-danger').show();
		            	}     	   	 	 
		            },
		            error: function(xhr, textStatus, thrownError) {
